...
class SLAM:
    def load(self, mapfile, scanfile):
        self.map = [
            [float(v) for v in l.split(",")] for l in open(mapfile, "r").readlines()
        ]
    ...
    def visual(self):
        ...
        for i in range(11):
            curve(
                pos=[(stepy * i + miny, 0, minx), (i * stepy + miny, 0, maxx)],
                color=color.blue,
            )
            curve(
                pos=[(miny, 0, stepx * i + minx), (maxy, 0, i * stepx + minx)],
                color=color.blue,
            )
        # axes
        ...
        while true:
            ...
            if drag:
                if alt:
                    ...
                    scanframe.rotate(
                        angle=-self.angle, axis=(0, 1, 0), origin=scanframe.pos
                    )
                    if vec1.z == 0:
                        vec1.z = 1.0e-10
...
class ExecEvaluator:
    ...
    def __call__(self, genotype):
        ...
        if type(genotype) == str:
            ...
            alpha = self.zmin + float(int(zbstr, 2)) * (self.zmax - self.zmin) / (
                2 ** len(zbstr) - 1
            )
            phenotype = (x, y, alpha)
        else:
